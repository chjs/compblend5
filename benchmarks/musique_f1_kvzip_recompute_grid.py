...
def _make_entry_at_ratio(cmp_full, target_ratio, n_kv_heads, head_dim):
    """Build a KVStore entry at the requested kvzip ratio.

    Critical fix (2026-05-29): K, V, and importance are kept INTACT — only
    valid_mask reflects the eviction. CompBlend-old's BlendingCache works
    this way too (see https://github.com/chjs/CompBlend-old/blob/main/src/compblend/cache.py).

    Why this matters: the fuser uses K_stored for two things —
      (1) the HKVD score `‖K_fresh - K_stored‖²` at the check layer, and
      (2) the cached K at non-top positions in every sparse layer.
    If K_stored is zero-filled at evicted positions, (1) gets a wildly
    inflated deviation at evicted slots (= ‖K_fresh‖²), and (2) feeds 0
    into attention at every cached-evicted position even when the per-head
    mask would otherwise let some heads attend there.

    Correct behavior: keep K_stored = original K everywhere. Use
    valid_mask only via the SDPA per-head mask (which excludes evicted
    positions from softmax) and via the selector's eligible_mask.
    """
    del n_kv_heads, head_dim  # unused — K, V kept as-is, no reshape needed
